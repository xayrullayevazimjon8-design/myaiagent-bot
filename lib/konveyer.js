...
function tugmalar(id, bosh) {
  ...
  return [
    ...
    [{ text: '✏️ Qayta yoz', callback_data: d('q') }, { text: '❌ Bekor', callback_data: d('b') }],
  ];
}
...
export async function izohBilanQayta(chatId, id, izoh) {
  ...
  await sendMessage(chatId, izohJarayonMatni(hukm));
  await korsat(chatId, { ...q, post, rasm: q.fileId, mime: 'image/png' });
}
