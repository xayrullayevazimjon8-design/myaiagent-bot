...
export async function ask(userText, tarix = []) {
  ...
  for (let qadam = 0; qadam <= MAX_VOSITA; qadam++) {
    ...
    // Modelning o'z chiqishini kiritishga qaytaramiz — chaqiruv va natija
    // bir zanjirda turishi kerak. Fikrlash qadamlari qaytarilmaydi.
    qadamlar.push(...javobQadamlari.filter(
      (q) => q.type === 'model_output' || q.type === 'function_call',
    ));
