...
export async function izohBilanQaytaYoz({ mavzu, natija, post }, izoh, { byudjetMs = BYUDJET_MS } = {}) {
  ...
  const yangi = await yozuvchi.izohBilanYoz(mavzu, natija, post, izoh, {
    timeoutMs: Math.max(ENG_KAM_MS, qolgan()),
  });
  if (qolgan() <= 0) return { post: yangi, hukm: null };

  const hukm = await muharrir.tekshir(mavzu, natija, yangi, {
    timeoutMs: Math.max(ENG_KAM_MS, qolgan()),
  });
...
export function izohJarayonMatni(hukm) {
  if (!hukm) return '🧪 Muharrir: tekshiruvga vaqt qolmadi.';
  if (hukm.otdi) return '🧪 Muharrir: o\'tdi.';
  return `🧪 Muharrirning fikri (post qaytarilmadi, qaror sizniki):\n${qisqa(hukm.sabab)}`;
}
